...
- **智能解析**：
  - 自動處理千分位逗號（如 45,800，需用雙引號包住：\"45,800\"）
  - 自動清除貨幣符號（如 $32,000）
  - 自動處理雙引號（RFC 4180 規範）
  - 自動跳過表頭列（如「姓名」、「金額」等關鍵字）
- **同名加總**：匯入時自動合併同名項目
- **雙模式支援**：根據勾選狀態自動切換「含姓名」或「純金額」模式
- **來源標籤**：匯入成功後顯示「資料來源由 {filename} 匯入」
- **驗證規則**：
  - 最多匯入 1000 筆資料（與手動輸入限制一致）
  - 單筆金額上限 999,999（與手動輸入限制一致）
  - 檔案大小上限 1MB
  - 詳細錯誤訊息（含行號、原始值、姓名）
...
- 範例金額用雙引號包住（如："45,800"），符合 RFC 4180 標準
...
- **第三方套件** (v0.3.0 新增)：
  - **PapaParse v5.4.1**：CSV 解析與產生引擎（via CDN）
    - 完美支援 RFC 4180 規範
    - 自動處理雙引號與千分位逗號
    - 零外部依賴，純前端執行
  - **html2canvas**：圖片匯出功能
...
- **匯出功能**：
  ...
  - 自動加入時間浮水印
  - 隱藏按鈕與時間戳（僅顯示計算結果）
  - **CSV 匯出** (v0.3.0 新增)：使用 PapaParse 產生符合 RFC 4180 的標準格式
...
當前版本：**v0.3.0** (2026-08-09)
- 新增 CSV 匯入功能（整合 PapaParse）
- 新增 CSV 範例下載（UTF-8 BOM）
- 新增 CSV 匯出功能（含個人明細與總計列）
- 強化錯誤處理與使用者回饋
